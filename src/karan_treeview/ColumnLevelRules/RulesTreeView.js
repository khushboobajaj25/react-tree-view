...
import { getTreeViewData, removeHiddenObjs, removeUncheckedObjs } from "../utils";
...
function RulesTreeView({ draggedItem, setDraggedItem, setEditorData }) {
  ...
  const tempAutoGeneratedRules = JSON.parse(JSON.stringify(autoGeneratedRules));
  removeHiddenObjs(tempAutoGeneratedRules);
  const columnLevelRules = getTreeViewData(tempAutoGeneratedRules);
  ...
  return (
    <div style={{ display: "flex",gap:'5px' }}>
      ...
      <div className="checkbox" style={{ overflowX: "auto", flex: 3 }}>
        <TreeView
          // isFocused={false}
          data={data}
          focusedId={-1}
          ...
          nodeRenderer={({ element, isBranch, isExpanded, isDisabled, getNodeProps, level, handleSelect, handleExpand,onKeyDown }) => {
            ...
            return (
              ...
              >
                ...
                {element.metadata.id === 1 && <div>
                <FilterView listOfColumns={autoGeneratedRules["List_of_columns"]} />
                </div>}
              </div>
            );
          }}
